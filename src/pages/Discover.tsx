...
const Discover = () => {
  ...
  const handlePendingRouteAction = useCallback((routeData: any) => {
    ...
    toast.success(
      'Route ready to save! Please use the Save Route button in the route panel.'
    );
  }, []);
...
useEffect(() => {
  ...
  const filteredPlaques = useMemo(() => {
    ...
    let filtered = allPlaques.filter((plaque) => {
      ...
      const matchesOrganisation =
        urlState.organisations.length === 0 ||
        (() => {
    ...
    if (distanceFilter.enabled && distanceFilter.center) {
      ...
      console.log(
        `After distance filter: ${filtered.length} (was ${beforeDistance})`
      );
    }
  ...
  const handleDistanceFilterChange = useCallback(
    (newFilter: Partial<DistanceFilter>) => {
  ...
  const getDistanceFromActiveLocation = useCallback(
    (plaque: Plaque) => {
      if (
        !distanceFilter.enabled ||
        !distanceFilter.center ||
  ...
  const getNearbyPlaques = useCallback(
    (currentPlaque: Plaque) => {
      return allPlaques
        .filter(
          (p) =>
            p.id !== currentPlaque.id &&
            (p.postcode === currentPlaque.postcode ||
              p.profession === currentPlaque.profession)
        )
        .slice(0, 3);
    },
    [allPlaques]
      ...
      return urlState.view === 'map' ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="h-48 md:h-64 bg-gray-100 rounded-lg animate-pulse"
            ></div>
          ))}
        </div>
      );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
            {paginatedPlaques.map((plaque) => (
              <PlaqueCard
                ...
                distance={
                  distanceFilter.enabled
                    ? getDistanceFromActiveLocation(plaque)
                    : 0
                }
                {...commonProps}
              />
  ...
  return (
    ...
    >
      <div
        className={`discover-content ${layoutClasses.container} ${isKeyboardOpen ? 'keyboard-open' : ''}`}
      >
          ...
          {urlState.view === 'map' ? (
            ...
            <div className="h-full overflow-y-auto">
              <div className="container mx-auto px-4 py-3 md:py-4">
                {/* Results header */}
                <div
                  className={`flex justify-between items-center mb-3 md:mb-4 ${isMobile() ? 'pr-20' : ''}`}
                >
                ...
                <div className="pb-20 md:pb-8">{renderContent()}</div>
              </div>
            </div>
          )}
        </div>
        ...
        {selectedPlaque && (
          <PlaqueDetail
            ...
            distance={
              distanceFilter.enabled
                ? getDistanceFromActiveLocation(selectedPlaque)
                : undefined
            }
            formatDistance={formatDistance}
            showDistance={distanceFilter.enabled}
