...
import { MapPin, Star, CheckCircle, MoreHorizontal, Pencil, Copy, Share2, Trash } from 'lucide-react';
...
const EnhancedCollectionCard = ({
  ...
  // Extract the color name from the bg class, with safety checks
  const getColorClass = () => {
  ...
  // Safely get color name for gradient
  const getColorName = () => {
    if (!collection.color) return 'blue';
    if (typeof collection.color === 'string') {
      // If it has bg- prefix, extract the color name
  ...
  return (
    ...
    >
      ...
      <div className="h-40 relative overflow-hidden">
        ...
        {/* Decorative elements */}
        <div className={`absolute top-4 right-4 w-24 h-24 rounded-full ${colorClass} opacity-10`}></div>
        <div className={`absolute bottom-8 left-8 w-16 h-16 rounded-full ${colorClass} opacity-5`}></div>
        ...
        <div className={`absolute top-6 left-6 w-14 h-14 rounded-2xl flex items-center justify-center text-white text-3xl ${colorClass} shadow-md`}>
          {collection.icon}
        </div>
