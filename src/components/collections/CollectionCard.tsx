...
const CollectionCard: React.FC<CollectionCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`${mobile ? 'h-3' : 'h-2'} w-full ${collection.color}`}
      ></div>
      ...
      <div className={mobile ? 'p-5' : 'p-4'}>
        ...
        <div className="flex justify-between items-start mb-3">
          <div className="flex items-center gap-3">
            <div
              className={`${mobile ? 'h-16 w-16' : 'h-12 w-12'} rounded-lg ${collection.color} flex items-center justify-center text-white ${mobile ? 'text-3xl' : 'text-2xl'}`}
            >
              {collection.icon}
            </div>
            <div className="min-w-0 flex-1">
              <h3
                className={`font-semibold ${mobile ? 'text-xl' : 'text-lg'} text-gray-800 group-hover:text-blue-600 transition-colors line-clamp-1`}
              >
                {collection.name}
              </h3>
          ...
          <div className="flex items-center">
            ...
            {isSelected && (
              <div
                className={`bg-blue-100 text-blue-600 p-1 rounded-full mr-2 ${mobile ? 'p-2' : ''}`}
              >
        ...
        {collection.description && (
          <p
            className={`${mobile ? 'text-base' : 'text-sm'} text-gray-600 mb-4 line-clamp-2`}
          >
            {collection.description}
          </p>
        )}
