  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (propAvailablePlaques && propAvailablePlaques.length > 0) {
        results = propAvailablePlaques.filter((plaque) =>
          searchInPlaque(plaque, searchQuery)
        );
      } else {
        ...
        for (let i = 0; i < dataArray.length; i += chunkSize) {
          ...
          const chunkResults = adaptedChunk.filter((plaque) =>
            searchInPlaque(plaque, query)
          );
  ...
  const renderPlaqueItem = (plaque: Plaque, isInSelectedView = false) => {
    ...
    return (
      <div
        ...
        onClick={() =>
          !isInSelectedView && !isAlreadyInCollection && togglePlaque(plaque)
        }
      >
        <div className={`flex items-center ${mobile ? 'p-4' : 'p-3'}`}>
          {/* Plaque image */}
          <div
            className={`${mobile ? 'w-16 h-16' : 'w-12 h-12'} rounded overflow-hidden flex-shrink-0 bg-gray-100`}
          >
            <PlaqueImage
          ...
          <div className="ml-3 overflow-hidden pr-3 flex-grow min-w-0">
            <h4
              className={`font-medium ${mobile ? 'text-base' : 'text-sm'} line-clamp-2`}
            >
              {plaque.title}
            </h4>

            <p
              className={`${mobile ? 'text-sm' : 'text-xs'} text-gray-500 line-clamp-2`}
            >
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {plaque.color && (
                <Badge
                  variant="outline"
                  className={`${mobile ? 'text-xs' : 'text-xs'} py-0`}
                >
                  {plaque.color}
                </Badge>
              )}
              ...
              {isAlreadyInCollection && (
                <Badge
                  variant="outline"
                  className="text-xs py-0 bg-green-100 text-green-700 border-green-200"
                >
                  Added
                </Badge>
              )}
          ...
          >
            {isInSelectedView ? (
              ...
              <div
                className={`${mobile ? 'w-8 h-8' : 'w-6 h-6'} rounded-full flex items-center justify-center bg-green-100 text-green-600 border border-green-300`}
              >
                <CheckCircle size={mobile ? 16 : 14} />
              </div>
            ) : (
              <div
                className={`${mobile ? 'w-8 h-8' : 'w-6 h-6'} rounded-full flex items-center justify-center`}
                style={{
              ...
              >
                {isSelected && (
                  <Check size={mobile ? 16 : 14} className="text-white" />
                )}
              </div>
            )}
          </div>
      ...
      >
        ...
        <div className="flex flex-col h-full">
          ...
          <div className={`${mobile ? 'p-4' : 'p-6'} pb-2`}>
            ...
            <div
              className={`mt-2 ${mobile ? 'text-sm' : 'text-sm'} text-gray-500 bg-blue-50 p-4 rounded-md border border-blue-100`}
            >
              <p className="flex items-center gap-2">
                <MapPin size={16} className="text-blue-500 flex-shrink-0" />
                <span>
                  You can also add plaques directly from the{' '}
                  <strong>Discover</strong> page when browsing the map or list
                  view.
                </span>
              </p>
            </div>
          ...
          <div
            className={`${mobile ? 'p-4' : 'p-6'} pt-2 pb-0 flex-grow overflow-hidden`}
          >
            <div
                    ...
                    <div className="flex-grow flex items-center justify-center py-8 text-center">
                      <div className="flex flex-col items-center gap-3 max-w-sm">
                        ...
                        <p className="text-gray-500">
                          Search for plaques to add to your collection
                        </p>
                        <p className="text-xs text-gray-400">
                          You can search by title, location, profession, or any
                          other text related to the plaques
                        </p>
                      </div>
                    </div>
                    ...
                    <>
                      {searchResults.length >= 100 && (
                        <div className="mb-3 text-xs flex gap-2 items-center bg-amber-50 p-2 rounded-md border border-amber-200">
                          ...
                          />
                          <span>
                            Showing up to 100 matching plaques. Refine your
                  ...
                  {selectedPlaqueIds.length === 0 ? (
                    <div className="flex-grow flex items-center justify-center py-8 text-center">
                      <div className="flex flex-col items-center gap-3 max-w-sm">
                        ...
                        <p className="text-xs text-gray-400">
                          Switch to the Search tab to find and select plaques to
          ...
          <div
            ...
            style={{
              paddingBottom: mobile
                ? Math.max(safeArea.bottom + 10, 20)
                : undefined,
            }}
          >
