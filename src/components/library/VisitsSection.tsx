...
const VisitsSection: React.FC<VisitsSectionProps> = ({
  ...
  const recentVisits = [...visits]
    .sort((a, b) => {
      const dateA = a.visited_at?.toDate
        ? a.visited_at.toDate()
        : new Date(a.visited_at);
      const dateB = b.visited_at?.toDate
        ? b.visited_at.toDate()
  ...
  const thisMonthVisits = visits.filter((visit) => {
    const visitDate = visit.visited_at?.toDate
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 sm:space-y-3">
            {visitsWithDetails.map((visit, index) => (
              ...
              >
                <div
                  className={`bg-${visit.plaque.color || 'blue'}-100 text-${visit.plaque.color || 'blue'}-600 w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform`}
                >
