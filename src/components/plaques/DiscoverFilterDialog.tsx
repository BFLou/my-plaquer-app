...
export const DiscoverFilterDialog: React.FC<DiscoverFilterDialogProps> = ({
  ...
  const enhancedFilterOptions = useMemo(() => {
      ...
      return originalOptions.map((option) => {
        const filteredCount = distanceFilteredPlaques.filter((plaque) => {
          ...
          if (isOrganisation) {
            ...
            if (orgsStr === '' || orgsStr === '[]' || orgsStr === 'Unknown')
              return false;
          ...
          if (fieldName === 'color') {
            const fieldStr = getFieldValueAsString(
              plaque[fieldName as keyof Plaque]
            );
            return (
              fieldStr.toLowerCase().trim() ===
              option.value.toLowerCase().trim()
            );
          }
          ...
          return (
            fieldStr.toLowerCase().trim() === option.value.toLowerCase().trim()
          );
        }).length;
    ...
    return {
      ...
      organisations: calculateFilteredCounts(
        organisations,
        'organisations',
        true
      ),
  ...
  const filteredToggleCounts = useMemo(() => {
    ...
    const visited = distanceFilteredPlaques.filter((plaque) => {
      ...
      const userVisited = isPlaqueVisited
        ? isPlaqueVisited(plaque.id) === true
        : false;
    ...
    >
          ...
          <Alert className="border-blue-200">
            ...
            <AlertDescription className="text-sm text-blue-800">
              ...
              <span className="font-medium">
                {distanceFilteredPlaques.length} of {allPlaques.length} plaques
                in this area.
              </span>
            </AlertDescription>
          </Alert>
      ...
      <div className="space-y-3 mb-6">
        {filterCategories.map((category) => {
          const availableOptionsCount = category.options.filter(
            (opt) => !isOptionDisabled(opt)
          ).length;
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <Switch
                ...
                disabled={
                  distanceFilter?.enabled && filteredToggleCounts.visited === 0
                }
                className="scale-125"
              />
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
              ...
              <Switch
                ...
                disabled={
                  distanceFilter?.enabled &&
                  filteredToggleCounts.favorites === 0
                }
                className="scale-125"
              />
            </div>
  ...
  const renderCategoryView = (category: FilterCategory) => {
    const { options, selectedValues, onChange, showColors, searchable } =
      category;
    ...
    const visibleOptions = showAll
      ? filteredOptions
      : filteredOptions.slice(0, DEFAULT_ITEMS_TO_SHOW);
        ...
        <div className="flex justify-between mb-4">
          ...
          <MobileButton
            ...
            onClick={() =>
              onChange(
                filteredOptions
                  .filter((o) => !isOptionDisabled(o))
                  .map((o) => o.value)
              )
            }
            disabled={
              selectedValues.length ===
              filteredOptions.filter((o) => !isOptionDisabled(o)).length
            }
          >
            Select Available
          </MobileButton>
              ...
              return showColors ? (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      !disabled &&
                      toggleSelection(selectedValues, onChange, option.value)
                    }
                    disabled={disabled}
                  />
          ...
          filteredOptions.length === 0 && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="text-xs text-yellow-800">
                <strong>Debug Info:</strong> No options available for{' '}
                {category.name}
                <br />
                ...
                {distanceFilter?.enabled && (
                  <>
                    <br />
                    Distance filter: {distanceFilter.radius}km from{' '}
  ...
  return (
    <MobileDialog
      ...
      description={
        currentView === 'main' && distanceFilter?.enabled
