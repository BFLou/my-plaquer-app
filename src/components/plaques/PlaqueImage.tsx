...
export const PlaqueImage: React.FC<PlaqueImageProps> = ({
  ...
  if (error || !hasValidSrc) {
    return (
      ...
      >
        <div
          className={`${getColorClass()} w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold`}
        >
          {getInitials()}
        </div>
      </div>
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {loading && (
        <div
          className={`absolute inset-0 ${placeholderClassName} flex items-center justify-center`}
        >
