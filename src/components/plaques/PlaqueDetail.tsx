// src/components/plaques/PlaqueDetail.tsx - Enhanced with collection integration
...
import { useCollections } from '@/hooks/useCollection'; // NEW: Import collections hook
...
export const PlaqueDetail: React.FC<PlaqueDetailProps> = ({
  ...
  const { collections } = useCollections(); // NEW: Get all collections
  ...
  const [showAllCollections, setShowAllCollections] = useState(false); // NEW: State for showing all collections
  ...
  // NEW: Find collections containing this plaque
  const plaqueCollections = useMemo(() => {
    if (!collections || !plaque) return [];
    
    return collections.filter(collection => {
      // Handle both array and number for plaques field
  ...
  // Check if value is valid (not empty, "Unknown", etc.)
  ...
  const handleShare = async (method: 'native' | 'copy') => {
    const shareData = {
      ...
      url: `${window.location.origin}/plaque/${plaque.id}`
    };
          ...
          <div className="h-full flex flex-col overflow-hidden max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto max-h-[calc(90vh-14rem)]">
              <div className="p-6 space-y-4">
                ...
                {/* NEW: Collections Preview - Only show if plaque is in actual collections */}
