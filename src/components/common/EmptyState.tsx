...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  const renderVisual = () => {
    if (icon) {
      return <div className="mb-4 text-6xl opacity-50">{icon}</div>;
    }
  ...
  const renderDescription = () => {
    ...
    if (hasFilters) {
      return (
        <p className="text-gray-600 mb-8 max-w-lg text-lg leading-relaxed">
          No plaques match your current filters. Try adjusting your filters to
          see more results.
        </p>
      );
    }
  ...
  return (
    ...
    >
      ...
      {/* Action Button */}
      {actionButton && (
        <div className="flex flex-col sm:flex-row gap-4">{actionButton}</div>
      )}
