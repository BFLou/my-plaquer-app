...
import { 
  Search, MapPin, Filter, Lightbulb, RefreshCw, 
  Navigation, TrendingUp, Users, Map
} from 'lucide-react';
...
export const SmartEmptyState: React.FC<SmartEmptyStateProps> = ({
  ...
  const getEmptyStateContent = () => {
    const hasFilters = filterContext.colors.length > 0 || 
    ...
    if (filterContext.search && !filterContext.search.match(/^[A-Z]{1,2}\d{1,2}[A-Z]?\s?\d[A-Z]{2}$/i)) {
      return {
    ...
    if (filterContext.search && filterContext.search.match(/^[A-Z]{1,2}\d{1,2}[A-Z]?\s?\d[A-Z]{2}$/i)) {
      return {
        ...
        description: "This postcode area doesn't have any recorded plaques in our database.",
        icon: <MapPin className="w-16 h-16 text-gray-400" />,
        suggestions: [
          {
            label: "Search Nearby Areas",
            action: () => onSuggestAction('expandPostcode', { postcode: filterContext.search }),
    ...
    if (filterContext.colors.length + filterContext.postcodes.length + 
        filterContext.professions.length + filterContext.organisations.length > 3) {
      return {
    ...
    if (filterContext.distanceFilter?.enabled && filterContext.distanceFilter.locationName) {
        ...
        suggestions: [
          {
            ...
            action: () => onSuggestAction('expandDistance', { 
        ...
        suggestions: [
          {
            ...
            action: () => onRemoveFilter(filterContext.onlyVisited ? 'onlyVisited' : 'onlyFavorites'),
            icon: filterContext.onlyVisited ? <Users size={16} /> : <TrendingUp size={16} />,
            variant: "default" as const
          },
          {
  ...
  const getActiveFilterCount = () => {
    return filterContext.colors.length + 
  ...
  return (
    <div className={`flex flex-col items-center justify-center py-12 px-6 text-center max-w-2xl mx-auto ${className}`}>
      {/* Icon */}
      <div className="mb-6">
        {content.icon}
      </div>
      ...
      {getActiveFilterCount() > 0 && (
        <div className="mt-6 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="flex flex-wrap gap-1 justify-center">
            ...
            {filterContext.distanceFilter?.enabled && filterContext.distanceFilter.locationName && (
              <Badge variant="outline" className="text-xs">
