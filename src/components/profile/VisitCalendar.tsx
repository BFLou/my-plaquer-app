...
const VisitCalendar: React.FC<VisitCalendarProps> = ({
  ...
  const buildCalendarDays = (): CalendarDay[] => {
    ...
    const endOfCalendar = endOfWeek(new Date(startOfCalendar.getTime() + 41 * 24 * 60 * 60 * 1000));
    ...
    <div className={`bg-white rounded-lg shadow-sm border ${className}`}>
          ...
          {calendarDays.map((day, index) => {
            ...
            return (
              <button 
                ...
                `}
                onClick={() => day.date && handleDateClick(day.date, day.hasVisits)}
                disabled={isDisabled || isLoading}
                aria-label={day.date ? `${format(day.date, 'MMMM d, yyyy')}${day.hasVisits ? ` - ${day.visitCount} visit${day.visitCount !== 1 ? 's' : ''}` : ''}` : undefined}
              >
                {day.day && (
                  <>
                    ...
                    {day.hasVisits && (
                      <div className="absolute bottom-0.5 right-0.5">
                        ...
                        `}>
                          <span className="hidden sm:inline">{day.visitCount}</span>
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-gray-500">
                  {new Set(visits.filter(visit => {
                    const visitDate = normalizeDate(visit.visited_at);
                    return visitDate && isSameMonth(visitDate, currentMonth);
                  }).map(v => v.plaque_id)).size} unique plaques
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-3 sm:p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge variant="secondary" className="text-xs">
                {visitsOnSelectedDate.length} visit{visitsOnSelectedDate.length !== 1 ? 's' : ''}
              </Badge>
            </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2 mb-1">
                          ...
                          {visit.rating && (
                            <div className="flex items-center gap-1 flex-shrink-0">
                              ...
                              <span className="text-xs text-amber-600 font-medium">{visit.rating}</span>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-500">
                          ...
                          {visit.photos && visit.photos.length > 0 && (
                            <div className="flex items-center gap-1">
                              <span>📷</span>
                              <span>{visit.photos.length} photo{visit.photos.length !== 1 ? 's' : ''}</span>
                            </div>
                          )}
                        </div>
            ...
            <div className="mt-3 pt-3 border-t border-gray-200">
              <div className="flex items-center justify-between">
                <span className="text-xs text-gray-500">
                  {visitsOnSelectedDate.length} visit{visitsOnSelectedDate.length !== 1 ? 's' : ''} on this day
                </span>
                <Button
                  variant="ghost"
                  size="sm"
      ...
      {!hasVisitsThisMonth && !isLoading && (
        <div className="p-4 text-center text-gray-500">
          ...
          <p className="text-sm">No visits in {format(currentMonth, 'MMMM yyyy')}</p>
        </div>
      )}
