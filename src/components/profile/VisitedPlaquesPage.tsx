...
const VisitedPlaquesPage: React.FC<VisitedPlaquesPageProps> = ({
  ...
  const getFilteredAndSortedVisits = () => {
    ...
    return filtered.sort((a, b) => {
      switch(sortOrder) {
        case 'recent':
          const dateA = a.visited_at instanceof Date ? a.visited_at : 
                      (a.visited_at?.toDate ? a.visited_at.toDate() : new Date(a.visited_at));
          const dateB = b.visited_at instanceof Date ? b.visited_at : 
                      (b.visited_at?.toDate ? b.visited_at.toDate() : new Date(b.visited_at));
          return dateB.getTime() - dateA.getTime();
          
        case 'oldest':
          const dateC = a.visited_at instanceof Date ? a.visited_at : 
                      (a.visited_at?.toDate ? a.visited_at.toDate() : new Date(a.visited_at));
          const dateD = b.visited_at instanceof Date ? b.visited_at : 
                      (b.visited_at?.toDate ? b.visited_at.toDate() : new Date(b.visited_at));
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-lg">
          ...
          <p className="text-gray-500 mb-6 max-w-md mx-auto">
            Start exploring and visiting plaques around London. Mark plaques as visited to track your discoveries.
          </p>
          <Button 
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <h3 className="text-lg font-medium text-gray-700 mb-2">No Matching Visits</h3>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className={`bg-${plaque?.color || 'blue'}-100 text-${plaque?.color || 'blue'}-500 w-12 h-12 rounded-lg flex items-center justify-center`}>
                    <MapPin size={20} />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center text-sm text-gray-500 mt-1">
                      ...
                      <span>Visited on {formatVisitDate(visit.visited_at)}</span>
                    </div>
                    {plaque?.location && (
                      <p className="text-sm text-gray-600 mt-1 truncate">{plaque.location}</p>
                    )}
                    {visit.notes && (
                      <p className="text-sm text-gray-600 mt-1 italic truncate">"{visit.notes}"</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            return (
              ...
              >
                <div className="flex flex-col items-center text-center">
                  <div className={`bg-${plaque?.color || 'blue'}-100 text-${plaque?.color || 'blue'}-500 w-16 h-16 rounded-lg flex items-center justify-center mb-3`}>
                    <MapPin size={24} />
                  </div>
                  
                  <h4 className="font-medium text-base">{plaque?.title || `Plaque #${visit.plaque_id}`}</h4>
                  <p className="text-sm text-gray-500 flex items-center mt-1">
                    <Calendar size={12} className="mr-1" />
                    {formatVisitDate(visit.visited_at)}
                  </p>
                  {plaque?.location && (
                    <p className="text-xs text-gray-600 mt-1 truncate">{plaque.location}</p>
                  )}
                </div>
              </div>
            );
