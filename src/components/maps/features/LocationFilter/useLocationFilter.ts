...
export const useLocationFilter = () => {
  ...
  const findMyLocation = useCallback(async (): Promise<
    [number, number] | null
  > => {
  ...
  const setManualLocation = useCallback(
    async (address: string): Promise<[number, number] | null> => {
      ...
      try {
        ...
        const searchQuery = address.includes('London')
          ? address
          : `${address}, London, UK`;
        ...
        if (data && data.length > 0) {
          ...
          if (
            ...
            toast.success(
              `Location found: ${place.display_name.split(',')[0]}`
            );
          }
  ...
  const searchWithSuggestions = useCallback(
    async (query: string): Promise<LocationResult[]> => {
      ...
      try {
        const searchQuery = query.includes('London')
          ? query
          : `${query}, London`;
        ...
        return data.map((place: any) => ({
          coordinates: [parseFloat(place.lat), parseFloat(place.lon)] as [
            number,
            number,
          ],
  ...
  const reverseGeocode = useCallback(
    async (coords: [number, number]): Promise<string | null> => {
      try {
        const response = await fetch(
  ...
  const calculateDistance = useCallback(
    (coords1: [number, number], coords2: [number, number]): number => {
