...
export const createPlaqueIcon = (
  ...
  const color =
    colorMap[(plaque.color?.toLowerCase() || 'blue').toLowerCase()] ||
    '#3b82f6';
...
export const createPlaquePopup = (
  ...
  const setupEventHandlers = () => {
    const detailButton = popupContent.querySelector(
      `#${detailBtnId}`
    ) as HTMLButtonElement;
    const routeButton = popupContent.querySelector(
      `#${routeBtnId}`
    ) as HTMLButtonElement;
    ...
    if (routeButton && onAddToRoute && isRoutingMode) {
      const handleRouteClick = (e: Event) => {
        ...
        debouncer(
          ...
          500
        ); // 500ms debounce for route additions
      };
...
export function getPlaqueMarkerColor(plaque: Plaque): string {
  ...
  if (plaque.profession) {
    ...
    if (
      profession.includes('author') ||
      profession.includes('writer') ||
      profession.includes('poet')
    ) {
      return '#8b5cf6'; // violet-500
    }

    if (
      profession.includes('artist') ||
      profession.includes('painter') ||
      profession.includes('sculptor')
    ) {
      return '#ec4899'; // pink-500
    }

    if (
      profession.includes('scient') ||
      profession.includes('physic') ||
      profession.includes('math')
    ) {
      return '#14b8a6'; // teal-500
    }

    if (
      profession.includes('polit') ||
      profession.includes('leader') ||
