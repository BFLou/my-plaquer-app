...
export const RouteStats: React.FC<RouteStatsProps> = ({
  ...
  const getDifficulty = () => {
    if (totalDistance < 2 && totalWaypoints < 5)
      return { level: 'Easy', color: 'bg-green-100 text-green-800' };
    if (totalDistance < 5 && totalWaypoints < 10)
  ...
  const postcodes = [
    ...new Set(plaques.map((p) => p.postcode).filter(Boolean)),
  ];
            ...
            <div className="space-y-2">
              <div className="text-sm text-gray-600">
                ...
                <span className="font-medium">
                  {totalWaypoints > 1
                    ? (totalDistance / (totalWaypoints - 1)).toFixed(2)
                    : '0'}{' '}
                  km
                </span>
              </div>
