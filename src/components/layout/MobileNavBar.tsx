...
export const MobileNavBar: React.FC = () => {
  ...
  const [selectedAuthItem, setSelectedAuthItem] = React.useState<NavItem | null>(null);
  ...
  const handleQuickSignIn = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toast.success(`Welcome! Accessing your ${selectedAuthItem.label.toLowerCase()}`);
      }, 100);
          ...
          {selectedAuthItem && (
            <div className="bg-blue-50 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <h3 className="font-medium text-blue-900">{selectedAuthItem.label}</h3>
                  <p className="text-sm text-blue-700">{selectedAuthItem.description}</p>
                </div>
              </div>
            </div>
          )}
