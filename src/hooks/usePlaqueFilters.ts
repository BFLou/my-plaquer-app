...
export const usePlaqueFilters = (allPlaques: Plaque[], options: UsePlaqueFiltersOptions = {}) => {
  // Filter states
  const [searchQuery, setSearchQuery] = useState<string>(options.initialSearchQuery || '');
  const [selectedPostcodes, setSelectedPostcodes] = useState<string[]>(options.initialPostcodes || []);
  const [selectedColors, setSelectedColors] = useState<string[]>(options.initialColors || []);
  const [selectedProfessions, setSelectedProfessions] = useState<string[]>(options.initialProfessions || []);
  const [onlyVisited, setOnlyVisited] = useState<boolean>(options.initialOnlyVisited || false);
  const [onlyFavorites, setOnlyFavorites] = useState<boolean>(options.initialOnlyFavorites || false);
  const [sortOption, setSortOption] = useState<string>(options.initialSortOption || 'newest');
  const [favorites, setFavorites] = useState<number[]>(options.initialFavorites || []);
  ...
  const filteredPlaques = useMemo(() => {
    return allPlaques.filter((plaque) => {
      ...
      const matchesSearch = 
        ...
        (plaque.location?.toLowerCase().includes(searchQuery.toLowerCase()) || false);
  ...
  ], [selectedPostcodes, selectedColors, selectedProfessions, onlyVisited, onlyFavorites]);
