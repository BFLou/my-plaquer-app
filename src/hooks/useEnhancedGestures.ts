      ...
      if (touches.length === 1) {
        ...
        if (handlers.onLongPress) {
          touchState.current.longPressTimer = setTimeout(() => {
            const currentTouch = touchState.current.touches.get(
              touch.identifier
            );
            if (currentTouch) {
  ...
  const handleTouchMove = useCallback(
    (event: TouchEvent) => {
      ...
      if (touches.length === 1) {
        ...
        const touch = touches[0];
        const originalTouch = Array.from(
          touchState.current.touches.values()
        )[0];
        ...
        if (originalTouch) {
          ...
          if (
            ...
            moveDistance > opts.dragThreshold
          ) {
    ...
    (event: TouchEvent) => {
      ...
      if (remainingTouches.length === 0) {
        ...
        if (touchState.current.isDragging) {
          ...
          !touchState.current.preventNextTap
        ) {
          // Single tap
          ...
          if (originalTouch && duration < opts.tapTimeout) {
            ...
            if (moveDistance < opts.dragThreshold) {
              ...
              const timeSinceLastTap =
                currentTime - touchState.current.lastTapTime;
              ...
              if (
                moveDistance > opts.swipeThreshold &&
