...
export const useAuthGate = () => {
  ...
  const storeNavigationContext = useCallback(
    (options: AuthGateOptions = {}) => {
      ...
      if (options.preserveModal) {
        ...
        context.modalState = {
          plaqueId: params.get('plaque')
            ? parseInt(params.get('plaque')!)
            : undefined,
  ...
  const retrieveNavigationContext =
    useCallback((): NavigationContext | null => {
      try {
  ...
  const requireAuth = useCallback(
    (
      ...
      if (!user) {
        ...
        const authGateUrl = getAuthGateUrl(
          pendingActionData,
          location.pathname
        );
  ...
  const getAuthGateUrl = useCallback(
    (pendingActionData?: PendingAction, currentPath?: string) => {
      if (pendingActionData) {
  ...
  const requireAuthForFavorite = useCallback(
    (plaqueId: number, favoriteAction: () => void) => {
      requireAuth(
        favoriteAction,
        {
  ...
  const requireAuthForCollection = useCallback(
    (plaqueId: number, collectionAction: () => void) => {
      requireAuth(
        collectionAction,
        {
  ...
  const requireAuthForRoute = useCallback(
    (routeData: any, routeAction: () => void) => {
      requireAuth(
        routeAction,
        {
