...
export const calculatePlaqueCounts = (inputData: unknown): PlaqueCounts => {
  ...
  const matchesProfessionCategory = (profession: string, category: string): boolean => {
    const profLower = profession.toLowerCase();
    const categoryMappings = [
      { 
        category: 'Famous Authors', 
        keywords: ['poet', 'novelist', 'writer', 'author', 'playwright', 'songwriter'] 
      },
      { 
        category: 'Artists', 
        keywords: ['artist', 'painter', 'sculptor', 'artiste', 'portrait painter', 'landscape painter'] 
      },
      { 
        category: 'Musicians', 
      ...
      { 
        ...
        keywords: ['civil engineer', 'engineer', 'scientist', 'inventor', 'astronomer', 'naturalist', 'physicist', 'chemist', 'biologist', 'mathematician', 'botanist', 'researcher'] 
      },
      { 
        category: 'Politicians & Leaders', 
        keywords: ['statesman', 'politician', 'prime minister', 'campaigner', 'activist', 'mayor', 'anti-slavery'] 
      },
      { 
        category: 'Architects', 
      ...
      { 
        ...
        keywords: ['doctor', 'physician', 'surgeon', 'nurse', 'medical', 'medicine'] 
      }
    ];
    
    const mapping = categoryMappings.find(m => m.category === category);
    return mapping ? mapping.keywords.some(keyword => profLower.includes(keyword)) : false;
  };

  // Helper function to check if organisation matches category using categoryMappings  
  const matchesOrganisationCategory = (organisations: string, category: string): boolean => {
  ...
  return [
    {
      ...
      onClick: () =>
        navigate('/discover?professions=poet,novelist,writer,author,playwright,songwriter&view=grid'),
    },
    {
      label: 'Artists',
      icon: '🎨',
      ...
      onClick: () =>
        navigate('/discover?professions=artist,painter,sculptor,music%20hall%20artiste,portrait%20painter,landscape%20painter&view=grid'),
    },
    {
      label: 'Musicians',
      icon: '🎵',
      ...
      onClick: () =>
        navigate('/discover?professions=composer,musician,singer,conductor,jazz%20musician&view=grid'),
    },
    {
      ...
      onClick: () =>
        navigate('/discover?professions=civil%20engineer,scientist,engineer,inventor,astronomer,naturalist,physicist,chemist,biologist,mathematician&view=grid'),
    },
    {
      ...
      onClick: () =>
        navigate('/discover?professions=statesman,politician,Prime%20Minister%20of%20the%20United%20Kingdom,anti-slavery%20campaigner,campaigner%20for%20women\'s%20rights,Mayor%20of%20London,campaigner,activist&view=grid'),
    },
    {
      label: 'Architects',
      icon: '🏗️',
      ...
      onClick: () =>
        navigate('/discover?professions=architect,theatre%20architect,Fellow%20of%20the%20Royal%20Institute%20of%20British%20Architects,landscape%20architect,Gothic%20architect&view=grid'),
    },
    {
      ...
      onClick: () =>
        navigate('/discover?professions=Doctor%20-%20Bachelor%20of%20Medicine%20MBBS,physician,surgeon,Doctor%20(unknown%20type),Doctor%20of%20Medicine,nurse&view=grid'),
    },
    {
      label: 'Blue Plaques',
      icon: '🔵',
      ...
      onClick: () =>
        navigate('/discover?colors=blue&view=grid'),
    },
    {
      label: 'Green Plaques',
      icon: '🟢',
...
export const getPlaqueSummary = (counts: PlaqueCounts): string => {
  const total = counts.famousAuthors + counts.artists + counts.musicians + counts.scientistsAndInventors + 
                counts.politiciansAndLeaders + counts.architects + counts.medicalProfessionals;
...
export const getMostPopularCategory = (counts: PlaqueCounts): { category: string; count: number } => {
  const categories = [
    ...
    { category: 'Scientists & Inventors', count: counts.scientistsAndInventors },
...
export const getCategoryFilterSuggestions = (category: string): string[] => {
  const categoryFilters: Record<string, string[]> = {
    'Famous Authors': ['poet', 'novelist', 'writer', 'author', 'playwright', 'songwriter'],
    'Artists': ['artist', 'painter', 'sculptor', 'music hall artiste', 'portrait painter', 'landscape painter'],
    'Musicians': ['composer', 'musician', 'singer', 'conductor', 'jazz musician'],
    'Scientists & Inventors': ['civil engineer', 'scientist', 'engineer', 'inventor', 'astronomer', 'naturalist', 'physicist', 'chemist', 'biologist', 'mathematician'],
    'Politicians & Leaders': ['statesman', 'politician', 'prime minister', 'anti-slavery campaigner', 'campaigner', 'activist'],
    'Architects': ['architect', 'theatre architect', 'landscape architect', 'gothic architect'],
...
export const getCategoryFromProfession = (profession: string): string | null => {
  ...
  const categoryMappings = [
    ...
    { 
      category: 'Scientists & Inventors', 
      keywords: ['engineer', 'scientist', 'inventor', 'astronomer', 'naturalist', 'physicist', 'chemist', 'biologist', 'mathematician'] 
    },
    { 
      category: 'Politicians & Leaders', 
      keywords: ['statesman', 'politician', 'prime minister', 'campaigner', 'activist', 'mayor'] 
    },
    { 
      category: 'Architects', 
      keywords: ['architect'] 
...
export const getCategoryFromOrganisation = (organisations: string): string | null => {
  const orgsLower = organisations.toLowerCase();
