  ...
  try {
    ...
    const routePoints: RoutePoint[] = points.map((plaque, index) => {
      ...
      if (typeof plaque.latitude === 'string') {
        ...
        throw new Error(
          `Invalid latitude for plaque ${plaque.id}: ${plaque.latitude}`
        );
      }
      ...
      if (typeof plaque.longitude === 'string') {
        ...
        throw new Error(
          `Invalid longitude for plaque ${plaque.id}: ${plaque.longitude}`
        );
      }

      if (isNaN(lat) || isNaN(lng)) {
        throw new Error(
          `Invalid coordinates for plaque ${plaque.id}: ${lat}, ${lng}`
        );
      }
    ...
    const missingFields = requiredFields.filter(
      (field) => !documentKeys.includes(field)
    );
    ...
    const extraFields = documentKeys.filter(
      (field) => !allAllowedFields.includes(field)
    );
    if (extraFields.length > 0) {
      console.warn(
        '⚠️ Extra fields found (might cause security rule issues):',
        extraFields
      );
    ...
    if (
      typeof routeData.name !== 'string' ||
      routeData.name.length === 0 ||
      routeData.name.length > 100
    ) {
      return {
        success: false,
        error: {
    ...
    if (
      typeof routeData.user_id !== 'string' ||
      routeData.user_id.length === 0
    ) {
      return {
        success: false,
        error: {
    ...
    if (error && typeof error === 'object' && 'message' in error) {
      ...
      if (
        ...
        errorObj.code?.includes('permission')
      ) {
...
export async function getRouteStats(userId: string): Promise<
  ServiceResult<{
    totalRoutes: number;
    totalDistance: number;
    totalPlaques: number;
